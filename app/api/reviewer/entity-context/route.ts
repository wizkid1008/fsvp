...
export async function GET(req: NextRequest) {
  ...
  if (type === "facility") {
    const [entityRes, docsRes] = await Promise.all([
      (admin.from("facilities_verify") as any)
        .select("id, facility_name, facility_type, facility_address_json, fda_registration_number, food_safety_certifications, approval_status, supplier_id, suppliers(company_name)")
        .eq("id", id)
        .maybeSingle(),
  ...
  const [entityRes, docsRes] = await Promise.all([
    (admin.from("products_verify") as any)
      .select("id, product_name, country_of_origin, raw_or_processed, intended_use, approval_status, supplier_id, suppliers(company_name), facilities_verify(facility_name)")
      .eq("id", id)
      .maybeSingle(),
    (admin.from("documents") as any)
      .select("id, title, document_kind, evidence_status, uploaded_at, expiration_date")
