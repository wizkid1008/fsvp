...
export default async function ReviewerPage() {
  ...
  const { data: rawDocs } = await (admin.from("documents") as any)
    .select(`
  ...
  const importerIds  = [...new Set(docs.map((d) => d.importer_id).filter(Boolean))] as string[];
  ...
  const [suppliersRes, itemsRes, profilesRes, facilitiesRes, importersRes, productsRes] = await Promise.all([
    ...
    facilityIds.length > 0
      ? (admin.from("facilities_verify") as any).select("id, facility_name, supplier_id").in("id", facilityIds)
      : Promise.resolve({ data: [] }),
  ...
  const facilityMap = new Map(
    ((facilitiesRes.data ?? []) as Array<{ id: string; facility_name: string; supplier_id: string }>)
      .map((f) => [f.id, f.facility_name])
  );
