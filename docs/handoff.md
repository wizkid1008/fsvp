...
1. **Scheduled maintenance exists, but its production configuration is
   unconfirmed.** `.github/workflows/scheduled-compliance.yml` calls
   ...
   reassessments, expiring documents and long-open corrective actions. It is
   disabled unless `INGEST_TRIGGER_SECRET` is present as a Cloudflare runtime
   binding, and the workflow also needs repository secrets
   `INGEST_TRIGGER_SECRET` and `FSVP_BASE_URL`. Whether that was ever done is
   recorded nowhere — check the workflow's run history first.
...
3. **Reference layer rules are still empty.** The commodity taxonomy is seeded
   (migration 018), which was safe because a taxonomy asserts nothing.
   `country_commodity_rules` is not, so admissibility correctly answers "pending
   review" for everything. Seeding needs real APHIS citations, entered as drafts
   for a second person to verify.
...
1. Confirm scheduled maintenance is configured and running in Cloudflare and
   GitHub, and that it advances each FDA source and creates alerts in
   production.
