...
1. **Scheduled maintenance has never once succeeded — cause established
   2026-09-02.** `.github/workflows/scheduled-compliance.yml` calls
   `/api/cron/compliance` daily. The route runs one bounded FDA ingest window
   for the requested source and calls `public.generate_compliance_alerts()` for
   reassessments, expiring documents and long-open corrective actions.

   It is scheduled and it fires: 21 runs on `main` since 2026-08-13, every one
   triggered by cron, every one **failed**. Run 1 failed, so this has never
   worked at any point.

   Earlier versions of this entry could not say which of the script's three
   exits was firing, because the logs need a GitHub login. It does not need the
   logs. Ask the deployment:
   ...
   It answers `404 {"error":"Scheduled ingest is not configured."}`. That is
   `configuredSecret()` in `lib/auth/trigger-secret.ts` returning null:
   **`INGEST_TRIGGER_SECRET` is unset in Cloudflare Pages**, or set to a
   placeholder containing `xxxxx`, which that function treats as unset. The
   route disables itself, `curl --fail-with-body` sees a non-2xx, the step
   exits 1. So it is the third exit, not the first two — §2's silent-binding
   failure, as this entry previously guessed, though it returns 404 rather than
   the 401 guessed: the route disables itself entirely rather than rejecting a
   secret.

   Note what this does NOT establish. It proves the Cloudflare side is
   unconfigured; it says nothing about whether the two GitHub repository
   secrets are set. If `FSVP_BASE_URL` or the GitHub copy of
   `INGEST_TRIGGER_SECRET` is also missing, the script exits at a guard before
   curl is ever reached, and fixing Cloudflare alone would move the failure
   rather than end it. Set the Cloudflare binding first, then re-run with
   `workflow_dispatch` — one minute to find out, against a day of waiting.
   ...
   existed. That has cost nothing yet only because there is no live data
   (§ "no production data"); it would cost a great deal the week there is.

   Worth fixing alongside it: 21 identical red runs told nobody which of three
   causes it was. The workflow discards the HTTP status and body it already
   receives. Echoing them would have named this in one run instead of three
   weeks.
